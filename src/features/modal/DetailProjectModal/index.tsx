...
const DetailProjectModal = ({ visible, setVisible, rowData }: IModalProps) => {
  ...
  const [getPrjDetail, { data: project }] = useLazyGetProjectDetailQuery();

  const [upDateProject] = useUpDateProjectMgtListMutation();
  ...
  useEffect(() => {
    if (project?.projectInfo) {
      form.setFieldsValue({
        project_name: project.projectInfo[0].project_name,
        start_dt: dayjs(project.projectInfo[0].start_dt),
        end_dt: dayjs(project.projectInfo[0].end_dt),
        company: project.projectInfo[0].company,
        business_price: project.projectInfo[0].business_price,
        business_desc: project.projectInfo[0].business_desc,
        sales_manager: project.projectInfo[0].sales_manager,
        solution_charger: project.projectInfo[0].solution_charger,
        free_maintenance_start_dt: project.projectInfo[0]
          .free_maintenance_start_dt
          ? dayjs(project.projectInfo[0].free_maintenance_start_dt)
          : "",
        free_maintenance_end_dt: project.projectInfo[0].free_maintenance_end_dt
          ? dayjs(project.projectInfo[0].free_maintenance_end_dt)
          : "",
        solutions: project.solutionInfo?.map((solution) => ({
          nexcore_solution_cd: solution.nexcore_solution_cd,
          solution_price: solution.solution_price,
        })),
        pay_maintenance_yn: project.projectInfo[0].pay_maintenance_yn,
        pay_maintenance_price: project.projectInfo[0].pay_maintenance_price,
        regular_maintenance_state:
          project.projectInfo[0].regular_maintenance_state,
        customer_charger: project.projectInfo[0].customer_charger,
        project_status: project.projectInfo[0].project_status,
      });
    }
  }, [project]);

  const handleFinish = (v) => {
    const startDate = dayjs(v.start_dt).format("YYYY-MM-DD");
    const endDate = dayjs(v.end_dt).format("YYYY-MM-DD");
    const fStartDate = v.free_maintenance_start_dt
      ? dayjs(v.free_maintenance_start_dt).format("YYYY-MM-DD")
      : "";
    const fEndDate = v.free_maintenance_end_dt
      ? dayjs(v.free_maintenance_end_dt).format("YYYY-MM-DD")
      : "";

    console.log(fStartDate);

    const formData: ProjectAddReq = {
      project_no: project?.projectInfo[0].project_no,
      ...v,
      ...
      >
        <Form form={form} onFinish={handleFinish}>
          <SearchForm>
            ...
            <FormItem label="유상 유지보수 금액" name="pay_maintenance_price">
              <InputNumber
