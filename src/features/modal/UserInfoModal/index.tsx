...
const UserInfoModal = ({ visible, setVisible }: IModalProps) => {
    ...
    const [getProfile, { data: profileData, isFetching: profileIsFetching }] =
        useLazyGetProfileListQuery();
    ...
    useEffect(() => {
        if (profileData?.myInfo) {
            form.setFieldsValue({
                ...
                nexcore_solution_name:
                    profileData.myInfo[0]?.nexcore_solution_name,
